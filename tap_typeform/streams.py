...
def get_date_and_integer_fields(stream):
    ...
    for prop, json_schema in stream.schema.properties.items():
        ...
        if isinstance(_type, list) and "integer" in _type or _type == "integer":
...
def select_fields(mdata, obj):
    ...
    for key, value in obj.items():
        ...
        if field_metadata and (
            field_metadata.get("selected") is True
            or field_metadata.get("inclusion") == "automatic"
        ):
            new_obj[key] = value
    return new_obj
...
@sleep_and_retry
@limits(calls=1, period=1)  # 5 seconds needed to be padded by 1 second to work
...
def get_form(atx, form_id, start_date, end_date):
    LOGGER.info(
        "Forms query - form: {} start_date: {} end_date: {} ".format(
            form_id,
...
def sync_form_definition(atx, form_id):
    with singer.metrics.job_timer("form definition " + form_id):
        ...
        while True:
            if (time.monotonic() - start) >= MAX_METRIC_JOB_TIME:
                raise Exception(
                    "Metric job timeout ({} secs)".format(MAX_METRIC_JOB_TIME)
                )
    ...
    for row in data:
        # unpackage the subquestions
        subfields = row["properties"]["fields"]
        for subrow in subfields:
            definition_data_rows.append(
                {
                    "form_id": form_id,
                    "question_id": subrow["id"],
                    "title": subrow["title"],
                    "ref": subrow["ref"],
                }
            )
...
def write_forms_state(atx, form, date_to_resume):
    write_bookmark(
        atx.state, form, "date_to_resume", date_to_resume.to_datetime_string()
    )
    atx.write_state()
...
def sync_forms(atx):
    ...
    for form_id in atx.config.get("forms").split(","):
        ...
        #     should_sync_forms = should_sync_forms or (
        #         stream_name in atx.selected_stream_ids
        #     )
        ...
        if incremental_range == "daily":
            ...
            start_date = pendulum.parse(
                atx.config.get("start_date", s_d + datetime.timedelta(days=-1, hours=0))
            )
        elif incremental_range == "hourly":
            s_d = now.replace(minute=0, second=0, microsecond=0)
            start_date = pendulum.parse(
                atx.config.get("start_date", s_d + datetime.timedelta(days=0, hours=-1))
            )
        elif incremental_range == "monthly":
            s_d = now.replace(minute=0, second=0, microsecond=0)
            start_date = pendulum.parse(
                atx.config.get(
                    "start_date", s_d + datetime.timedelta(days=30, hours=-0)
                )
            )
        ...
        if incremental_range == "daily":
            e_d = now.replace(hour=0, minute=0, second=0, microsecond=0).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            end_date = pendulum.parse(atx.config.get("end_date", e_d))
        elif incremental_range == "hourly":
            e_d = now.replace(minute=0, second=0, microsecond=0).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            end_date = pendulum.parse(atx.config.get("end_date", e_d))
        elif incremental_range == "monthly":
        ...
        while current_date <= end_date:
            if incremental_range == "daily":
                ...
                next_date = current_date + datetime.timedelta(days=30, hours=0)
            ...
            [responses, max_submitted_at] = sync_form(
                atx, form_id, ut_current_date, ut_next_date
            )
